...
*/		////////////// Mapper Input /////////////         ----> 변수 <value>에 담겨온다.
		...
		String journal_no_temp = "";// 현재 파싱하고있는 논문의 논문번호를 담기 위한 변수
		String year_temp = ""; // 현재 파싱하고있는 논문의 년도를 담기위한 변수
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			while (itr.hasMoreTokens()) //itr을 끝까지 순회한다.
			{
				...
				String test[] = ngram_set.split(" "); 
				int n = test.length; // ngram_set에 단어가 몇개들어갔는지 검사.
				
				if(n == ngram)//n그램 개수 유효성 조건
				{
					if (ngram_set.contains("http://") == false && ngram_set.contains("year:") == false && ngram_set.contains("journal_no:") == false
							&& ngram_set.equals("") == false) //링크나 체커들등 단어가 아닌것들을 골라서 보낸다.
					{
						Text word = new Text(); // word에 담아서 콤바이너한테 보낸다.
						word.set(year_temp + "\t" + ngram_set + "\t" + journal_no_temp); // ngram_set과 나온 년도와 논문번호를 컴바이너에게 보냄.    (2015   apple   4)(ex : 1gram)
